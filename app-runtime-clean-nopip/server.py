...
def _simulator_ensure_tables_on_start(industry: str) -> dict[str, Any]:
    """
    Best-effort refresh/fill path used by the Start simulator action.
    Keeps integration tables non-empty so OT/PI and ERP/SAP panels are usable
    immediately without manual backfill steps.
    """
    cfg = _industry_cfg(industry)
    catalog = str(cfg.get("catalog", f"pdm_{industry}") or f"pdm_{industry}")
    out: dict[str, Any] = {
        "industry": industry,
        "catalog": catalog,
        "silver_ot_pi_aligned_recent_rows": 0,
        "pi_simulated_tags_recent_rows": 0,
        "erp_bronze_work_orders_industry_rows": 0,
        "erp_bronze_cost_centers_industry_rows": 0,
        "lakebase_work_orders_sap_demo_rows": 0,
    }

    # Ensure ERP bronze tables exist without Delta default-column features.
    _warehouse_sql_exec(f"CREATE CATALOG IF NOT EXISTS {catalog}")
    _warehouse_sql_exec(f"CREATE SCHEMA IF NOT EXISTS {catalog}.bronze")
    _warehouse_sql_exec(
        f"""
        CREATE TABLE IF NOT EXISTS {catalog}.bronze.erp_bdc_work_orders (
          aufnr STRING,
          equipment_id STRING,
          site_id STRING,
          plant_code STRING,
    ...
    _warehouse_sql_exec(
        f"""
        CREATE TABLE IF NOT EXISTS {catalog}.bronze.erp_bdc_cost_centers (
          kostl STRING,
          industry STRING,
          site_id STRING,
          area_id STRING,
    ...
    # If silver OT↔PI has no recent rows, seed a fresh window from bronze OT↔PI.
    s_ot_pi = _run_sql(
        f"""
        SELECT
          SUM(CASE WHEN ot_timestamp >= current_timestamp() - INTERVAL 30 MINUTES THEN 1 ELSE 0 END) AS c
        FROM {catalog}.silver.ot_pi_aligned
        """,
        cache_key=f"{catalog}:simstart:silver_otpi:{int(time.time() // 5)}",
    )
    if int((s_ot_pi[0].get("c") if s_ot_pi else 0) or 0) <= 0:
        _warehouse_sql_exec(f"DELETE FROM {catalog}.silver.ot_pi_aligned WHERE data_source = 'sim_start_refresh'")
        _warehouse_sql_exec(
            f"""
            INSERT INTO {catalog}.silver.ot_pi_aligned
            SELECT
              site_id,
              area_id,
              unit_id,
              equipment_id,
              tag_name,
              current_timestamp() - ((rnk % 6) * INTERVAL 1 MINUTE) AS ot_timestamp,
              ot_value,
              ot_unit,
              COALESCE(ot_quality, 'good') AS ot_quality,
              current_timestamp() - ((rnk % 6) * INTERVAL 1 MINUTE) + INTERVAL 2 SECONDS AS pi_timestamp,
              COALESCE(pi_value, ot_value * 1.002) AS pi_value,
              COALESCE(pi_unit, ot_unit) AS pi_unit,
              COALESCE(pi_quality, ot_quality, 'good') AS pi_quality,
              2 AS time_delta_seconds,
              'sim_start_refresh' AS data_source,
              current_timestamp() AS _processed_at
            FROM (
              SELECT *,
                     row_number() OVER (ORDER BY ot_timestamp DESC) AS rnk
              FROM {catalog}.bronze.ot_pi_aligned
              LIMIT 240
            ) t
            """
        )

    # Ensure PI simulator bronze has fresh rows (using PI value, OT fallback).
    s_pi = _run_sql(
        f"""
        SELECT
          SUM(CASE WHEN timestamp >= current_timestamp() - INTERVAL 30 MINUTES THEN 1 ELSE 0 END) AS c
        FROM {catalog}.bronze.pi_simulated_tags
        """,
        cache_key=f"{catalog}:simstart:pi_tags:{int(time.time() // 5)}",
    )
    if int((s_pi[0].get("c") if s_pi else 0) or 0) <= 0:
        _warehouse_sql_exec(f"DELETE FROM {catalog}.bronze.pi_simulated_tags WHERE source_protocol = 'PI-SIM-REFRESH'")
        _warehouse_sql_exec(
            f"""
            ...
            SELECT
              ...
              tag_name,
              ROUND(COALESCE(pi_value, ot_value * 1.002), 6) AS value,
              COALESCE(pi_unit, ot_unit) AS unit,
              COALESCE(pi_quality, ot_quality, 'good') AS quality,
              '0' AS quality_code,
              'PI-SIM-REFRESH' AS source_protocol,
              current_timestamp() - ((rnk % 6) * INTERVAL 1 MINUTE) AS timestamp
            FROM (
              SELECT
                site_id, area_id, unit_id, equipment_id, tag_name,
                ot_value, ot_unit, ot_quality,
                pi_value, pi_unit, pi_quality,
                row_number() OVER (ORDER BY ot_timestamp DESC) AS rnk
              FROM {catalog}.silver.ot_pi_aligned
              WHERE ot_value IS NOT NULL OR pi_value IS NOT NULL
              LIMIT 240
            ) t
            """
        )

    # If ERP bronze demo rows are missing, hydrate from Lakebase ODS work orders.
    s_wo = _run_sql(
        f"SELECT COUNT(*) AS c FROM {catalog}.bronze.erp_bdc_work_orders WHERE industry = '{_sql_escape(industry)}'",
        cache_key=f"{catalog}:simstart:erp_wo:{int(time.time() // 10)}",
    )
    if int((s_wo[0].get("c") if s_wo else 0) or 0) <= 0:
        _warehouse_sql_exec(
            f"""
            INSERT INTO {catalog}.bronze.erp_bdc_work_orders
            SELECT
              CONCAT('00', UPPER('{_sql_escape(industry[:3])}'), LPAD(CAST(row_number() OVER (ORDER BY wo_id) AS STRING), 6, '0')) AS aufnr,
              equipment_id,
              site_id,
              COALESCE(plant_code, UPPER('{_sql_escape(industry[:3])}')) AS plant_code,
              COALESCE(work_center, 'WC-DEMO') AS work_center,
              COALESCE(cost_center, 'CC-DEMO') AS cost_center,
              'PM01' AS order_type,
              'REL' AS sap_system_status,
              UPPER(COALESCE(status, 'SCHEDULED')) AS sap_user_status,
              COALESCE(priority, 'P2') AS priority_sap,
              COALESCE(estimated_intervention_cost, intervention_cost, 0.0) AS planned_cost,
              COALESCE(intervention_cost, 0.0) AS actual_cost,
              COALESCE(amount_currency, 'USD') AS currency,
              COALESCE(description, CONCAT('ERP demo ', wo_id)) AS short_text,
              CONCAT('BDC-SESS-', UPPER('{_sql_escape(industry)}')) AS bdc_session_id,
              CONCAT('BDC-BATCH-', DATE_FORMAT(current_timestamp(), 'yyyyMMddHHmmss')) AS bdc_batch_id,
              '{_sql_escape(industry)}' AS industry
            FROM {catalog}.lakebase.work_orders
            WHERE source_system = 'SAP_BDC_DEMO'
            LIMIT 40
            """
        )
    ...
    _warehouse_sql_exec(
        ...
        SELECT
          CONCAT('K', LPAD(CAST(row_number() OVER (ORDER BY cost_center) AS STRING), 4, '0')) AS kostl,
          '{_sql_escape(industry)}' AS industry,
          COALESCE(site_id, 'unknown') AS site_id,
          'unknown' AS area_id,
          'unknown' AS unit_id,
          CONCAT('Cost Center ', COALESCE(cost_center, 'UNK')) AS cost_center_name,
          COALESCE(cost_center, 'UNK') AS responsible_cc,
          'USD' AS currency,
          1000000.0 AS annual_budget,
          250000.0 AS actual_ytd,
          0.25 AS variance_pct,
          CONCAT('BDC-SESS-', UPPER('{_sql_escape(industry)}')) AS bdc_session_id
        FROM (
          SELECT DISTINCT cost_center, site_id
          FROM {catalog}.bronze.erp_bdc_work_orders
          WHERE industry = '{_sql_escape(industry)}'
        ) d
        WHERE COALESCE(cost_center, 'UNK') NOT IN (
          SELECT COALESCE(responsible_cc, 'UNK')
          FROM {catalog}.bronze.erp_bdc_cost_centers
          WHERE industry = '{_sql_escape(industry)}'
        )
        """
    )

    # Summarize post-refresh status for UI and observability.
    s = _run_sql(
        f"""
        SELECT SUM(CASE WHEN ot_timestamp >= current_timestamp() - INTERVAL 30 MINUTES THEN 1 ELSE 0 END) AS c
        FROM {catalog}.silver.ot_pi_aligned
        """,
        cache_key=f"{catalog}:simstart:summary:otpi:{int(time.time() // 5)}",
    )
    out["silver_ot_pi_aligned_recent_rows"] = int((s[0].get("c") if s else 0) or 0)
    s = _run_sql(
        f"""
        SELECT SUM(CASE WHEN timestamp >= current_timestamp() - INTERVAL 30 MINUTES THEN 1 ELSE 0 END) AS c
        FROM {catalog}.bronze.pi_simulated_tags
        """,
        cache_key=f"{catalog}:simstart:summary:pi:{int(time.time() // 5)}",
    )
    out["pi_simulated_tags_recent_rows"] = int((s[0].get("c") if s else 0) or 0)
    s = _run_sql(
        f"SELECT COUNT(*) AS c FROM {catalog}.bronze.erp_bdc_work_orders WHERE industry = '{_sql_escape(industry)}'",
        cache_key=f"{catalog}:simstart:summary:wo:{int(time.time() // 10)}",
    )
    out["erp_bronze_work_orders_industry_rows"] = int((s[0].get("c") if s else 0) or 0)
    s = _run_sql(
        f"SELECT COUNT(*) AS c FROM {catalog}.bronze.erp_bdc_cost_centers WHERE industry = '{_sql_escape(industry)}'",
        cache_key=f"{catalog}:simstart:summary:cc:{int(time.time() // 10)}",
    )
    out["erp_bronze_cost_centers_industry_rows"] = int((s[0].get("c") if s else 0) or 0)
    s = _run_sql(
        f"SELECT COUNT(*) AS c FROM {catalog}.lakebase.work_orders WHERE source_system = 'SAP_BDC_DEMO'",
        cache_key=f"{catalog}:simstart:summary:sapdemo:{int(time.time() // 10)}",
    )
    out["lakebase_work_orders_sap_demo_rows"] = int((s[0].get("c") if s else 0) or 0)
    return out
...
def ui_simulator_control(payload: dict) -> dict:
    ...
    # Start action can proactively fill simulator-facing tables so users see data
    # right after clicking Start simulator.
    if action == "start" and bool(payload.get("ensure_tables", True)):
        warmup_ticks = max(0, min(12, int(payload.get("warmup_ticks", 3) or 3)))
        emitted = 0
        refresh_error = None
        try:
            ...
            out["table_refresh"] = _simulator_ensure_tables_on_start(industry)
        except Exception as e:
            refresh_error = str(e)
            st["last_error"] = refresh_error
        if refresh_error:
            out["table_refresh_error"] = refresh_error
